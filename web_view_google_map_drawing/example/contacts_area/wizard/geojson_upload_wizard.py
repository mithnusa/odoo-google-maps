...
class GeoJsonUploadWizard(models.TransientModel):
    ...
    name = fields.Char(
        string="Upload Name", required=True, default="GeoJSON Import"
    )
    ...
    def _onchange_geojson_file(self):
        ...
        if self.geojson_file:
            try:
                ...
                raise UserError(self.env._("Invalid JSON format: %s", e))
            except ValidationError as e:
                raise UserError(str(e))
            except Exception as e:
                _logger.error("Error processing GeoJSON file: %s", e)
                raise UserError(
                    self.env._("Error processing GeoJSON file: %s", e)
                )

    def _validate_geojson(self, geojson_data):
        """Validate GeoJSON structure"""
        if not isinstance(geojson_data, dict):
            raise ValidationError(self.env._("GeoJSON must be a JSON object"))

        if geojson_data.get("type") != "FeatureCollection":
            raise ValidationError(
                self.env._("GeoJSON must be a FeatureCollection")
            )

        features = geojson_data.get("features", [])
        if not features:
            raise ValidationError(
                self.env._("GeoJSON must contain at least one feature")
            )
        ...
        for i, feature in enumerate(features):
            if not isinstance(feature, dict):
                raise ValidationError(
                    self.env._("Feature %d is not a valid object", i + 1)
                )

            if feature.get("type") != "Feature":
                raise ValidationError(
                    self.env._("Feature %d is not a valid Feature type", i + 1)
                )

            geometry = feature.get("geometry")
            if not geometry or not geometry.get("type"):
                raise ValidationError(
                    self.env._("Feature %d has invalid geometry", i + 1)
                )
            ...
            if geometry.get("type") not in supported_types:
                _logger.warning(
                    "Feature %d has unsupported geometry type: %s",
                    i + 1,
                    geometry.get("type"),
                )
    ...
    def _create_preview_summary(self, features):
        ...
        for i, feature in enumerate(features[:10]):  # Show first 10 features
            ...
            geometry_counts[geometry_type] = (
                geometry_counts.get(geometry_type, 0) + 1
            )
    ...
    def _extract_feature_name(self, feature, index):
        ...
        if self.feature_name not in properties.keys():
            raise UserError(
                self.env._(
                    'The specified feature name source "%s" does not exist in feature properties. Please close the pop-up window and try again',
                    self.feature_name,
                )
            )
    ...
    def action_import_geojson(self):
        ...
        if not self.geojson_file:
            raise UserError(
                self.env._("Please select a GeoJSON file to upload")
            )
        ...
        try:
            ...
            raise UserError(self.env._("Error importing GeoJSON: %s", e))
        ...
        for i, feature in enumerate(features):
            try:
                ...
                if existing_record:
                    ...
                    _logger.info(
                        "Updated existing record: %s",
                        feature_data["gshape_name"],
                    )
                else:
                    # Create new record
                    ...
                    _logger.info(
                        "Created new record: %s", feature_data["gshape_name"]
                    )
    ...
    def _extract_feature_data(self, feature, index):
        ...
        feature_data = {
            ...
            "gshape_area": area,
            "gshape_description": self.default_description
