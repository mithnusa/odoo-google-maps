...
class GeoJsonUploadWizard(models.TransientModel):
    ...
    def _extract_feature_data(self, feature, index):
        ...
        # Map geometry types to gshape_type
        type_mapping = {
            'point': 'polygon',  # Points will be stored as polygons
            'linestring': 'polygon',  # Lines will be stored as polygons
            'polygon': 'polygon',
            'multipoint': 'polygon',
        ...
        gshape_type = type_mapping.get(geometry_type, 'polygon')
        
        # Calculate area and other measurements
        area = self._calculate_feature_area(feature)
        radius = self._extract_radius(feature)
        width, height = self._extract_dimensions(feature)
        
        # Prepare feature data
        feature_data = {
            'partner_id': self.env.user.partner_id.id,
            'gshape_name': feature_name,
            'gshape_type': gshape_type,
            'gshape_area': area,
        ...
        # Add any additional properties from the feature
        self._add_custom_properties(feature_data, properties)
    ...
    def _extract_radius(self, feature):
        """Extract radius for circle features"""
        properties = feature.get('properties', {})
        geometry = feature.get('geometry', {})
        
        if geometry.get('type') == 'Circle':
            return properties.get('radius', 0.0)
        
        return 0.0

    def _extract_dimensions(self, feature):
        """Extract width and height for rectangle features"""
        properties = feature.get('properties', {})
        geometry = feature.get('geometry', {})
    ...
    def _convert_to_paths_format(self, feature):
        """Convert GeoJSON feature to the legacy paths format"""
        # This method converts GeoJSON to the format expected by gshape_paths
        # You may need to adapt this based on your specific paths format
        geometry = feature.get('geometry', {})
        properties = feature.get('properties', {})
    ...
    def _add_custom_properties(self, feature_data, properties):
        """Add custom properties from GeoJSON to the record data"""
        # Map common GeoJSON properties to model fields
        property_mapping = {
            'name': 'gshape_name',
            'description': 'gshape_description',
            'area': 'gshape_area',
            'radius': 'gshape_radius',
            'width': 'gshape_width',
            'height': 'gshape_height',
        }
        
        for geojson_prop, model_field in property_mapping.items():
            if geojson_prop in properties and model_field not in feature_data:
                feature_data[model_field] = properties[geojson_prop]
