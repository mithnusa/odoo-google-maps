...
export class GoogleMapsDrawingSidebar extends GoogleMapSidebar {
    getData(record) {
        ...
        if (record.data.gshape_type === 'circle') {
            extras = [
                sprintf(_lt('Area: %s square meter'), record.data.gshape_area.toLocaleString()),
                sprintf(_lt('Radius: %s meter'), record.data.gshape_radius.toLocaleString()),
            ];
            ...
            extras = [
                sprintf(_lt('Area: %s square meter'), record.data.gshape_area.toLocaleString()),
                sprintf(_lt('Width: %s meter'), record.data.gshape_width.toLocaleString()),
                sprintf(_lt('Height: %s meter'), record.data.gshape_height.toLocaleString()),
            ];
        }
