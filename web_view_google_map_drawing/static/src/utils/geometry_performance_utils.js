...
export function simplifyCoordinates(
    coordinates,
    tolerance = GEOMETRY_PERFORMANCE_CONFIG.SIMPLIFICATION_TOLERANCE,
    minPoints = 2
) {
...
export function createUltraSimplifiedFeature(feature) {
    ...
    if (analysis.vertexCount > GEOMETRY_PERFORMANCE_CONFIG.TERRA_DRAW_FREEZE_THRESHOLD) {
        console.warn(
            `⚠️ Complex feature detected with ${analysis.vertexCount} vertices. Proceeding without simplification - Terra Draw may be slow.`
        );
        ...
        return {
            ...
            properties: {
                ...
                _warningMessage: sprintf(
                    _t('Complex feature with %s vertices - editing may be slow'),
...
export function isValidGeometry(geometry) {
    ...
    try {
        switch (geometry.type) {
            ...
            case 'MultiLineString':
                if (!Array.isArray(geometry.coordinates) || geometry.coordinates.length === 0) return false;
                return geometry.coordinates.every((line) => Array.isArray(line) && line.length >= 2);
...
export function createEditableFeature(feature, tolerance = GEOMETRY_PERFORMANCE_CONFIG.SIMPLIFICATION_TOLERANCE) {
    ...
    if (currentVertexCount > GEOMETRY_PERFORMANCE_CONFIG.TERRA_DRAW_FREEZE_THRESHOLD) {
        ...
        console.log(
            `⚠️ Extremely complex feature with ${currentVertexCount} vertices - starting with aggressive simplification`
        );
    }
    ...
    while (currentVertexCount > targetVertexCount && iterations < maxIterations) {
        ...
        if (!isValidGeometry(simplifiedFeature.geometry)) {
            console.warn(
                `⚠️ Iteration ${iterations}: Simplification with tolerance ${usedTolerance.toFixed(
                    4
        ...
        console.log(
            `Iteration ${iterations}: ${currentVertexCount} → ${newVertexCount} vertices (tolerance: ${usedTolerance.toFixed(
                4
            )})`
        );
        ...
        if (reductionRatio < 5 && iterations > 1) {
            ...
            console.log(
                `⚠️ Low reduction (${reductionRatio.toFixed(1)}%), increasing tolerance to ${usedTolerance.toFixed(4)}`
            );
        } else {
    ...
    if (finalVertexCount > targetVertexCount) {
        console.warn(
            `⚠️ Could not reach target vertex count after ${iterations} iterations. Final: ${finalVertexCount} (target: ${targetVertexCount})`
        );
    }
...
export function getPerformanceWarning(analysis) {
    ...
    const messages = {
        ...
        very_complex: {
            ...
            message: sprintf(
                _t('This feature has %s vertices. Terra Draw may freeze during editing.'),
                analysis.vertexCount
            ),
            suggestion: _t(
                'Strongly recommend simplifying or using the high-performance Deck.gl renderer for editing.'
            ),
        },
        extremely_complex: {
            ...
            message: sprintf(
                _t(
                    'This feature has %s vertices. It will be automatically simplified to preserve shape while maintaining Terra Draw performance.'
                ),
                analysis.vertexCount
            ),
            suggestion: _t(
                'The feature will be simplified while preserving its essential shape. Use Deck.gl renderer for full detail visualization.'
            ),
        },
    };
