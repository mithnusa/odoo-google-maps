...
import { useRef, onWillUpdateProps, onWillDestroy, useState, onRendered } from '@odoo/owl';
...
export class GoogleMapDrawingField extends BaseGoogleMapComponent {
     ...
     */
    _handleLoadShape() {
        console.log(' _handleLoadShape ');
        console.log(this);
        const value = this.props.record.data[this.props.name];
        if (value) {
            if (this.shapes.has(this.props.record.id)) {
                const shape = this.shapes.get(this.props.record.id);
    ...
    _storeInCache(shape) {
        ...
        this.shapes.set(this.props.record.id, shape);
    }
    ...
    _actionDelete() {
        if (this.selectedShape) {
            ...
            this.notificationService.add(_t('No shape selected to delete'), {
                type: 'warning',
            });
            this.notificationService.add(
                _t('Click on one of the shapes you have drawn to select a shape'),
                { type: 'info' }
            );
        }
    }
    ...
    _actionSave() {
        console.log(' _actionSave ');
        if (!this.selectedShape) {
            this.notificationService.add(_t('There is no shape to save'), {
                type: 'danger',
            });
            this.notificationService.add(
                _t(
    ...
    async _saveChanges(values) {
        ...
        if (values) {
            ...
            this.notificationService.add(_t('The shape has been recorded'), {
                type: 'info',
            });
            this.selectedShape = null;
            this.isEditing = false;
     ...
     */
    initializeDrawing() {
        if (!this.drawingManager && this.googleMap) {
    ...
    handleOverlayComplete(event) {
        ...
        if (this.selectedShape) {
            shape.setMap(null);
            this.notificationService.add(_t('Only one shape allowed'), {
                type: 'warning',
            });
            this.notificationService.add(
                _t(
    ...
    handleSetSelectedShape(shape) {
        console.log(' handleSetSelectedShape ');
        console.log(shape);
