    ...
    def _validate_tag_google_map(self, node, name_manager, node_info):
        ...
        if att_js_class and 'drawing' in att_js_class:
            if att_geojson is None:
                self._raise_view_error(_('Missing mandatory attribute for google_map_drawing view: "geojson"'), node)
            
            if att_lat or att_lng:
                self._log_view_warning(_('Attributes "lat" and "lng" should not be set when using "drawing" js_class in google_map view'), node)

        fields_name = [child.get('name') for child in node.iterchildren(tag=etree.Element) if child.tag == 'field']

        if att_geojson and not att_geojson in fields_name:
            self._raise_view_error(_('Field %(name)s assigned to attribute "geojson" does not exist. All fields used in google_map view attribute must be loaded', name=att_geojson), node)
