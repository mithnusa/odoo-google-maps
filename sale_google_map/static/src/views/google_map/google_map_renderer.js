...
export class GoogleMapRendererSales extends GoogleMapRenderer {
     ...
     */
    renderMarkers() {
        this.props.list.groups.map((group) => this.createMarkerIfNotExists(group));
    }
    ...
    _openCustomerForm(group) {
        const { relation } = group.groupByField;
        const resModel = relation ? relation : 'res.partner';
    ...
    _groupFindRecord(group) {
        function matchPartner(record) {
            ...
            if (record.data.partner_id) {
                isMatched = record.data.partner_id[0] === group.value;
            }
            return isMatched;
        }
        return group.records.find(matchPartner);
    }
    ...
    actionSeeSales(group) {
        ...
        if (actionId) {
            ...
            if (groupField) {
                domain.push([groupField, '=', group.value]);
            }
            let context = group.list.evalContext ? { ...group.list.evalContext } : {};
