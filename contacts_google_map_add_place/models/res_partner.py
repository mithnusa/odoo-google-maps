...
class ResPartner(models.Model):
    ...
    gplace_id = fields.Char(
        ...
        help="The unique identifier for the place in Google Maps. This is used to link the partner to a specific location on the map.",
        index=True,
        copy=False,
    )
    ...
    def _mapping_address(
        ...
        address_components,
        adr_format_address,
        field_mapping=ADDRESS_FIELDS_MAPPING,
    ):
        ...
        for field, mapping in field_mapping.items():
            ...
            for type_key in mapping:  # Follow mapping order
                ...
                if component:
                    ...
                    if not value:
                        value = component.get(old_type_keys.get(name_form, "short_name"))  # Fallback for old API versions
                    parts.append(value)
            if parts:
                values[field] = parts
    ...
    def action_in_map_google_place_create(self, place):
        ...
        if place_id:
            res_partner_id = self.env[self._name].search(
                [("gplace_id", "=", place_id)], limit=1
            )
            if res_partner_id:
                return {
                    "type": "ir.actions.act_window",
                    "name": _("Contact exists: %s", res_partner_id.name),
        ...
        values = self.default_get(USED_FIELDS)
        ...
        return {
            "type": "ir.actions.act_window",
            "name": _("New Contact from Google Place"),
            "res_model": "res.partner",
            "view_mode": "form",
            "view_id": False,
            "views": [(False, "form")],
    ...
    def _parse_formatted_address(self, formatted_address):
        ...
        **Russian/CIS style** (any length, last segment is all digits)::
        ...
        def _split_zip(segment):
            ...
            if match:
                name = segment[: match.start()].strip() or segment[match.end() :].strip()
