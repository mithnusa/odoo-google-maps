...
export class BaseGoogleMapRenderer extends BaseGoogleMapComponent {
    static template = 'web_base_view_google_map.GoogleMapRenderer';
    static components = {
        KanbanRecord,
    ...
    renderGeolocationData() {
        ...
        // Clean up obsolete records before processing new data
        this.cleanupObsoleteRecords(datas);

        if (this.props.list.isGrouped) {
            this.renderGroupedRecords(datas);
        } else {
            this.renderUngroupedRecords(datas);
        }
        this.debounceUpdateMap();
    }

    renderUngroupedRecords(datas) {
        this.renderRecords(datas);
    }

    async renderGroupedRecords(datas) {
        await Promise.all(
            datas.map(async ({ group }) => {
                try {
                    const records = await group.groupRecords();
                    const color = group.groupColor || '#E74C3C';
                    this.renderRecords(records, color);
                } catch (error) {
    ...
    renderRecords(datas, color) {
        // Performance optimization: Only process new or changed records
        let processedCount = 0;
        datas.forEach((record) => {
            let shouldProcessRecord = this.shouldProcessRecord(record);
            if (shouldProcessRecord) {
                const success = this.processRecord(record, color);
                if (success) {
                    processedCount++;
                    this.markRecordAsProcessed(record);
                }
            }
        });
    }

    processRecord(record, color) {}

    shouldProcessRecord(record) {
        const recordId = this.getRecordId(record);
        const currentChecksum = this.calculateRecordChecksum(record);
        const previousChecksum = this.recordChecksums.get(recordId);
        return !!(!this.processedRecords.has(recordId) || currentChecksum !== previousChecksum);
    }

    markRecordAsProcessed(record) {}

    cleanupObsoleteRecords(datas) {}
    ...
    getGroupsOrRecords() {
        ...
        if (list.isGrouped) {
            ...
            return list.groups.map((group, i) => ({
                id: isNull(group.value) ? `group_key_${i}` : `${String(group.value)}_${i}`,
                group: {
                    displayName: group.displayName,
                    count: group.count,
                    records: group.records || [],
                    groupColor: group.groupColor,
                    groupByLabel: group.groupByField?.label || 'Group',
                    groupRecords: async () => {
                        if (!group.isFolded) {
                            return group.records;
                        }
                        // Load group records if folded
                        await group.toggle();
                        return group.records;
                    },
                },
    ...
    get infoWindowTemplate() {
        return 'web_base_view_google_map.MarkerInfoWindow';
    }
     ...
     * @returns {string} The record ID as string
     */
    getRecordId(record) {
        return (record.resId || record.id).toString();
    }
}
