...
- **`_updateExistingMarker()` — Stale Cached `dataView`**: The `cacheRecordDataView` entry for an updated record was not invalidated, causing the cached (pre-update) coordinates to be returned on subsequent reads; added `cacheRecordDataView.delete(record.id)` before updating
...
- **`_renderMarkerForRecord()` — Cache Check Before Element Creation**: Moved the `cache.has(record.id)` early-return check before `_createMarkerElementValues()`, avoiding unnecessary DOM element creation for markers that will be updated rather than created
